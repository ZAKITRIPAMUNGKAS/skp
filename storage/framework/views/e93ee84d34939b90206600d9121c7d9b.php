...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-800 font-heading"><?php echo e($greeting); ?>, <?php echo e(auth()->user()->name); ?>! 👋</h1>
        ...
        <div class="flex items-center gap-3">
            <?php if(auth()->user()->isAdmin()): ?>
            <a href="<?php echo e(route('admin.events.create')); ?>" class="inline-flex items-center gap-2 px-5 py-2.5 bg-primary hover:bg-primary-600 text-white text-sm font-semibold rounded-xl transition-all shadow-sm">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-3xl border border-gray-100 p-8 shadow-sm relative overflow-hidden flex flex-col justify-between min-h-[350px]">
            <div class="absolute right-0 bottom-0 opacity-15 pointer-events-none translate-y-6 translate-x-6">
                <img src="<?php echo e(asset('images/arka/arka_fokus.png')); ?>" class="w-64 h-auto" alt="Mascot Arka" onerror="this.style.display='none'">
            </div>
            ...
            <div class="relative z-10 pt-6 flex flex-wrap items-center gap-3">
                <?php if($latestEvent): ?>
                    <a href="<?php echo e(route('admin.events.show', $latestEvent)); ?>" class="inline-flex items-center gap-2 px-6 py-3 bg-primary text-white text-sm font-bold rounded-2xl hover:bg-primary-600 transition-all shadow-md shadow-primary/20 hover:-translate-y-0.5">
                        Kelola Event Terakhir (<?php echo e(Str::limit($latestEvent->nama_event, 20)); ?>)
                ...
                <?php else: ?>
                    <?php if(auth()->user()->isAdmin()): ?>
                    <a href="<?php echo e(route('admin.events.create')); ?>" class="inline-flex items-center gap-2 px-6 py-3 bg-primary text-white text-sm font-bold rounded-2xl hover:bg-primary-600 transition-all shadow-md shadow-primary/20 hover:-translate-y-0.5">
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/></svg>
                        Buat Event Sekarang
                    </a>
                    <?php else: ?>
                    ...
                    <?php endif; ?>
                <?php endif; ?>
                <a href="<?php echo e(route('admin.events.index')); ?>" class="inline-flex items-center gap-2 px-6 py-3 bg-gray-50 border border-gray-200 text-gray-700 text-sm font-bold rounded-2xl hover:bg-gray-100 transition-all shadow-sm">
                    ...
                    <?php echo e(auth()->user()->isAdmin() ? 'Lihat Semua Event' : 'Daftar Event Saya'); ?>
        ...
        <div class="bg-gradient-to-br from-gray-900 to-slate-800 rounded-3xl p-8 text-white shadow-sm flex flex-col justify-center gap-6 relative overflow-hidden min-h-[350px]">
            ...
            <div class="space-y-3 relative z-10">
                ...
                <a href="<?php echo e(asset('MANUAL BOOK.pdf')); ?>" download="MANUAL BOOK.pdf" target="_blank" class="flex items-center gap-4 bg-white/5 border border-white/10 rounded-2xl p-4 hover:bg-white/10 hover:border-white/20 transition-all group">
                ...
                <a href="<?php echo e(route('admin.dashboard.video')); ?>" class="flex items-center gap-4 bg-white/5 border border-white/10 rounded-2xl p-4 hover:bg-white/10 hover:border-white/20 transition-all group">
                ...
                <a href="<?php echo e(route('admin.dashboard.documentation')); ?>" class="flex items-center gap-4 bg-white/5 border border-white/10 rounded-2xl p-4 hover:bg-white/10 hover:border-white/20 transition-all group">
...
<?php echo $__env->make('layouts.main', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\website\SKRIPSI\SISTEM\resources\views/admin/dashboard.blade.php ENDPATH**/ ?>