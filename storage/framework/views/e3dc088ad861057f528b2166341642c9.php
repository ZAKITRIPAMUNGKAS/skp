...
<div class="max-w-3xl mx-auto py-6 px-4">
    ...
    <?php if($activeEvent): ?>
        ...
        <div class="relative overflow-hidden bg-gradient-to-br from-[#1A6D9B] via-[#155C84] to-[#0B3A56] rounded-[2rem] p-7 text-white shadow-2xl shadow-primary/30 mb-8">
            ...
            <div class="relative flex flex-col sm:flex-row items-center gap-6">
                ...
                <div class="relative shrink-0 flex items-center justify-center"
                     x-data="{ pct: 0 }"
                     x-init="setTimeout(() => pct = <?php echo e($pct); ?>, 300)">
                    <svg width="96" height="96" viewBox="0 0 96 96">
                        
                        <circle cx="48" cy="48" r="<?php echo e($r); ?>" fill="none" stroke="rgba(255,255,255,0.12)" stroke-width="8"/>
                        
                        <circle cx="48" cy="48" r="<?php echo e($r); ?>" fill="none"
                                stroke="#D4A017"
                                stroke-width="8"
                                stroke-linecap="round"
                                stroke-dasharray="<?php echo e($circ); ?>"
                                :stroke-dashoffset="<?php echo e($circ); ?> - (<?php echo e($circ); ?> * pct / 100)"
                ...
                <div class="flex-1 text-center sm:text-left">
                    <span class="inline-block px-3 py-0.5 text-[10px] font-bold uppercase tracking-widest bg-white/10 border border-white/20 rounded-full text-white/90 mb-2">
                        <?php echo e($activeEvent->nama_event ?? 'Baitul Arqam'); ?>

                    </span>
                    <h2 class="text-3xl font-heading font-extrabold text-white leading-tight">
                        <?php echo e($attended); ?> <span class="text-white/50 font-light text-xl">/ <?php echo e($total); ?></span>
                    </h2>
                ...
                <div class="shrink-0 flex sm:flex-col gap-4 sm:gap-2">
                    <div class="text-center bg-white/10 backdrop-blur-sm border border-white/15 rounded-2xl px-5 py-3">
                        <span class="block text-xl font-bold text-white"><?php echo e($total); ?></span>
                        <span class="text-[10px] uppercase tracking-wide text-white/60">Total Sesi</span>
                    </div>
                    <div class="text-center bg-white/10 backdrop-blur-sm border border-white/15 rounded-2xl px-5 py-3">
                        <span class="block text-xl font-bold text-accent-300"><?php echo e($attended); ?></span>
            ...
            <div class="relative mt-6 h-2 bg-white/10 rounded-full overflow-hidden">
                <div class="absolute inset-y-0 left-0 rounded-full bg-gradient-to-r from-accent-400 to-accent-300 transition-all duration-1000"
                     x-data="{ w: 0 }"
                     x-init="setTimeout(() => w = <?php echo e($pct); ?>, 400)"
                     :style="'width:' + w + '%'"></div>
            </div>
            <p class="text-[11px] text-white/40 mt-2 text-right"><?php echo e($pct); ?>% kehadiran tercatat</p>
        </div>
        ...
        <div class="relative" x-data>
            ...
            <div class="space-y-4">
            <?php $__currentLoopData = $sessions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $sesi): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php
                    $delay = $index * 60;
                    $isAttended = $sesi['attended'];
                ?>

                <div class="sesi-card relative flex gap-4 group" style="animation-delay: <?php echo e($delay); ?>ms">
                    ...
                    <div class="relative z-10 shrink-0 flex flex-col items-center">
                        <div class="w-12 h-12 rounded-full flex items-center justify-center text-sm font-bold shadow-lg border-2 transition-all duration-300
                            ...
                            <?php else: ?>
                                <span class="font-heading"><?php echo e(str_pad($sesi['urutan'], 2, '0', STR_PAD_LEFT)); ?></span>
                            <?php endif; ?>
                        </div>
                    </div>
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-start gap-2 flex-wrap">
                                    <span class="shrink-0 inline-block px-2 py-0.5 text-[10px] font-bold uppercase rounded-md
                                        <?php echo e($isAttended ? 'bg-green-100 text-green-700' : 'bg-primary-50 text-primary-600'); ?>">
                                        Sesi <?php echo e(str_pad($sesi['urutan'], 2, '0', STR_PAD_LEFT)); ?>

                                    </span>
                                </div>
                                <h3 class="mt-2 text-base font-heading font-bold leading-snug
                                    <?php echo e($isAttended ? 'text-green-900' : 'text-gray-800 group-hover:text-primary'); ?>">
                                ...
                                <p class="mt-1 text-xs text-gray-500 leading-relaxed line-clamp-2">
                                    <?php echo e($sesi['deskripsi']); ?>

                                </p>
                                <?php endif; ?>

                                <?php if(!empty($sesi['pemateri'])): ?>
                                <p class="mt-1.5 text-xs font-medium text-gray-500 flex items-center gap-1.5">
                                    ...
                                    Pemateri: <span class="font-semibold text-gray-700"><?php echo e($sesi['pemateri']); ?></span>
                                </p>
                                <?php endif; ?>

                                <?php if(!empty($sesi['file_materi'])): ?>
                                <div class="mt-3">
                                    <a href="<?php echo e(asset('storage/' . $sesi['file_materi'])); ?>" target="_blank"
                                ...
                                <?php if(!empty($sesi['waktu_mulai']) || !empty($sesi['waktu_selesai'])): ?>
                                <div class="mt-2 flex items-center gap-2 flex-wrap">
                                    <?php if(!empty($sesi['waktu_mulai'])): ?>
                                    <span class="inline-flex items-center gap-1 text-[11px] text-gray-500">
                                        ...
                                        <?php if($sesi['waktu_mulai'] instanceof \Carbon\Carbon): ?>
                                            <?php echo e($sesi['waktu_mulai']->format('H:i')); ?>

                                        <?php else: ?>
                                            <?php echo e(\Carbon\Carbon::parse($sesi['waktu_mulai'])->format('H:i')); ?>

                                        <?php endif; ?>
                                        <?php if(!empty($sesi['waktu_selesai'])): ?>
                                            &ndash;
                                            <?php if($sesi['waktu_selesai'] instanceof \Carbon\Carbon): ?>
                                                <?php echo e($sesi['waktu_selesai']->format('H:i')); ?>

                                            <?php else: ?>
                                                <?php echo e(\Carbon\Carbon::parse($sesi['waktu_selesai'])->format('H:i')); ?>
                            ...
                            <div class="shrink-0 self-start sm:self-center">
                                <?php if($isAttended): ?>
                                    <div class="flex flex-col items-end gap-0.5 bg-green-100 border border-green-200 rounded-xl px-4 py-2.5">
                                        ...
                                        <?php if($sesi['waktu_scan']): ?>
                                            <span class="text-[10px] font-mono text-green-600 tabular-nums">
                                                <?php if($sesi['waktu_scan'] instanceof \Carbon\Carbon): ?>
                                                    <?php echo e($sesi['waktu_scan']->format('d M · H:i')); ?>

                                                <?php else: ?>
                                                    <?php echo e(\Carbon\Carbon::parse($sesi['waktu_scan'])->format('d M · H:i')); ?>
    ...
    <div class="flex flex-col items-center justify-center py-16 text-center bg-white border border-gray-100 rounded-[2.5rem] p-8 shadow-card">
        <div class="w-52 h-52 mb-6">
            <img src="<?php echo e(asset('images/arka/arka_fokus.png')); ?>" alt="Tidak ada acara" class="w-full h-full object-contain opacity-50 grayscale">
        </div>
        ...
        <a href="<?php echo e(route('peserta.dashboard')); ?>" class="mt-6 inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-primary text-white text-sm font-semibold hover:bg-primary-600 transition-colors shadow-md shadow-primary/20">
...
<?php echo $__env->make('layouts.main', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\website\SKRIPSI\SISTEM\resources\views/peserta/jadwal/index.blade.php ENDPATH**/ ?>