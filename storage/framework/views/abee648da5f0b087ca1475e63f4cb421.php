...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.empty-state','data' => ['title' => 'Belum ada sub-aspek','description' => 'Tambahkan sub-aspek afektif untuk evaluasi.','icon' => 'document']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => 'Belum ada sub-aspek','description' => 'Tambahkan sub-aspek afektif untuk evaluasi.','icon' => 'document']); ?>
...
function afektifManager() {
    return {
        subAspeks: <?php echo json_encode($subAspeks->map(fn($sa) => array_merge($sa->toArray(), ['_expanded' => false])), 512) ?>,
        ...
        async saveSubAspek() {
            ...
            if (this.editingSubAspek) {
                const url = '<?php echo e(route("admin.afektif.updateSubAspek", [$event, "__SA__"])); ?>'.replace('__SA__', this.editingSubAspek.id);
                const res = await fetch(url, { method: 'PUT', headers: {'Content-Type':'application/json','X-CSRF-TOKEN':'<?php echo e(csrf_token()); ?>'}, body: JSON.stringify({nama_sub_aspek: this.subAspekName}) });
                ...
                const res = await fetch('<?php echo e(route("admin.afektif.storeSubAspek", $event)); ?>', { method: 'POST', headers: {'Content-Type':'application/json','X-CSRF-TOKEN':'<?php echo e(csrf_token()); ?>'}, body: JSON.stringify({nama_sub_aspek: this.subAspekName}) });
                const data = await res.json();
        ...
        async deleteSubAspek(sa) {
            ...
            await fetch('<?php echo e(route("admin.afektif.destroySubAspek", [$event, "__SA__"])); ?>'.replace('__SA__', sa.id), { method: 'DELETE', headers: {'X-CSRF-TOKEN':'<?php echo e(csrf_token()); ?>'} });
            this.subAspeks = this.subAspeks.filter(s => s.id !== sa.id);
        },

        async toggleStatus(sa, newStatus) {
            await fetch('<?php echo e(route("admin.afektif.toggleStatus", [$event, "__SA__"])); ?>'.replace('__SA__', sa.id), { method: 'POST', headers: {'Content-Type':'application/json','X-CSRF-TOKEN':'<?php echo e(csrf_token()); ?>'}, body: JSON.stringify({status: newStatus}) });
            sa.status = newStatus;
        },
        ...
        async saveButir() {
            ...
            if (this.editingButir) {
                const res = await fetch('<?php echo e(route("admin.afektif.updateButir", [$event, "__B__"])); ?>'.replace('__B__', this.editingButir.id), { method: 'PUT', headers: {'Content-Type':'application/json','X-CSRF-TOKEN':'<?php echo e(csrf_token()); ?>'}, body: JSON.stringify(this.butirForm) });
                const data = await res.json();
                if (data.status === 'success') { Object.assign(this.editingButir, data.butir); }
            } else {
                const res = await fetch('<?php echo e(route("admin.afektif.storeButir", [$event, "__SA__"])); ?>'.replace('__SA__', this.activeSubAspek.id), { method: 'POST', headers: {'Content-Type':'application/json','X-CSRF-TOKEN':'<?php echo e(csrf_token()); ?>'}, body: JSON.stringify(this.butirForm) });
                const data = await res.json();
        ...
        async deleteButir(sa, b) {
            ...
            await fetch('<?php echo e(route("admin.afektif.destroyButir", [$event, "__B__"])); ?>'.replace('__B__', b.id), { method: 'DELETE', headers: {'X-CSRF-TOKEN':'<?php echo e(csrf_token()); ?>'} });
        ...
        async handleButirDrop(event, sa, targetButir) {
            ...
            await fetch('<?php echo e(route("admin.afektif.reorderButir", [$event, "__SA__"])); ?>'.replace('__SA__', sa.id), { method: 'POST', headers: {'Content-Type':'application/json','X-CSRF-TOKEN':'<?php echo e(csrf_token()); ?>'}, body: JSON.stringify({order}) });
        },
    };
}
</script>
<?php $__env->stopPush(); ?>
<?php /**PATH D:\website\SKRIPSI\SISTEM\resources\views/admin/events/partials/tab-afektif.blade.php ENDPATH**/ ?>