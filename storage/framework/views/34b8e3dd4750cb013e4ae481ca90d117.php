...
<div x-data="angketManager()" x-init="init()">
    ...
    <?php $__currentLoopData = ['A','B','C','D','E','F']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $kat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <div class="mb-4" x-show="itemsByKategori('<?php echo e($kat); ?>').length > 0">
            <div class="flex items-center gap-2 mb-2">
                <span class="px-2 py-0.5 bg-primary/10 text-primary text-xs font-bold rounded"><?php echo e($kat); ?></span>
                <span class="text-xs font-semibold text-gray-700"><?php echo e($categoryLabels[$kat] ?? ''); ?></span>
                <span class="text-xs text-gray-400" x-text="'(' + itemsByKategori('<?php echo e($kat); ?>').length + ' item)'"></span>
            </div>
            <div class="space-y-2">
                <template x-for="(item, idx) in itemsByKategori('<?php echo e($kat); ?>')" :key="item.id">
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.empty-state','data' => ['title' => 'Belum ada item angket','description' => 'Tambahkan item pertanyaan untuk angket.','icon' => 'document']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => 'Belum ada item angket','description' => 'Tambahkan item pertanyaan untuk angket.','icon' => 'document']); ?>
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <select x-model="form.kategori" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none">
                        <?php $__currentLoopData = $categoryLabels; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $k => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?> 
                            <option value="<?php echo e($k); ?>">Kategori <?php echo e($k); ?> (<?php echo e($label); ?>)</option> 
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </select>
                </div>
                <div>
...
function angketManager() {
    return {
        allItems: <?php echo json_encode($angketItems->flatten()->values(), 15, 512) ?>,
        ...
        async saveItem() {
            ...
            const url = this.editingItem
                ? '<?php echo e(route("admin.angket.updateItem", [$event, "__I__"])); ?>'.replace('__I__', this.editingItem.id)
                : '<?php echo e(route("admin.angket.storeItem", $event)); ?>';
        ...
        async deleteItem(item) {
            ...
            await fetch('<?php echo e(route("admin.angket.destroyItem", [$event, "__I__"])); ?>'.replace('__I__', item.id), { method: 'DELETE', headers: {'X-CSRF-TOKEN':'<?php echo e(csrf_token()); ?>'} });
...
<?php $__env->stopPush(); ?>
<?php /**PATH D:\website\SKRIPSI\SISTEM\resources\views/admin/events/partials/tab-angket.blade.php ENDPATH**/ ?>