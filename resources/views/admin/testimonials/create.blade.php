...
@section('breadcrumb')
    ...
    <svg class="w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
    <span class="text-sm font-medium text-gray-700">Tambah</span>
@endsection

@section('content')

    <x-page-header title="Tambah Testimoni" subtitle="Buat kutipan testimoni baru dari peserta / alumni Baitul Arqam">
        <x-slot:actions>
    ...
    <x-card>
        <form method="POST" action="{{ route('admin.testimonials.store') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <x-form-input label="Nama Lengkap" name="name" placeholder="Masukkan nama pemberi testimoni..." required="true" />

            <x-form-input label="Jabatan / Role / Instansi" name="role" placeholder="Contoh: Dosen FAI UMS, Tendik BAA UMS..." required="true" />

            <x-form-input label="Isi Testimoni" name="quote" type="textarea" rows="4" placeholder="Masukkan testimoni atau kutipan..." required="true" />

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">Foto Profil / Avatar</label>
                <label for="photo" class="relative flex items-center justify-center border-2 border-dashed border-gray-200 hover:border-primary rounded-2xl p-6 transition-all bg-gray-50/50 hover:bg-white cursor-pointer group"
                    ...
                    <div x-show="preview" class="w-full flex flex-col items-center gap-3" style="display: none;">
            ...
            <x-form-input label="Urutan Tampilan" name="urutan" type="number" value="1" placeholder="Contoh: 1, 2, 3..." required="true" helper="Menentukan urutan prioritas kemunculan testimoni di landing page." />
            ...
            <div class="flex items-center gap-3 justify-end border-t border-gray-100 pt-5 mt-8">
                ...
                <x-button type="submit" variant="primary">Simpan Testimoni</x-button>
            </div>
        </form>
    </x-card>
