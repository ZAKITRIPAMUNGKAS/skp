...
@section('title', 'Tambah Galeri Pelatihan')
...
@section('content')
    ...
    <x-page-header title="Tambah Foto Galeri" subtitle="Upload dokumentasi foto baru untuk kegiatan Baitul Arqam UMS">
        <x-slot:actions>
    ...
    <x-card>
        <form method="POST" action="{{ route('admin.galleries.store') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <x-form-input label="Judul Foto / Kegiatan" name="title" placeholder="Masukkan judul galeri..." required="true" />

            <x-form-input label="Nama Event (Opsional)" name="event_name" placeholder="Contoh: Baitul Arqam Dosen & Tendik..." />

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">File Gambar <span class="text-red-500">*</span></label>
            ...
            <x-form-input label="Urutan Tampilan" name="urutan" type="number" value="1" placeholder="Contoh: 1, 2, 3..." required="true" helper="Menentukan urutan prioritas kemunculan foto di landing page." />
            ...
            <div class="flex items-center gap-3 justify-end border-t border-gray-100 pt-5 mt-8">
                ...
                <x-button type="submit" variant="primary">Simpan Foto</x-button>
            </div>
        </form>
    </x-card>
