...
<head>
    <meta charset="UTF-8">
    <title>Cetak Masal ID Card – {{ $event->nama_event }}</title>
    <style>
        @page {
            margin: 0;
            size: 86mm 137mm;
...
@foreach($participants as $ep)
    ...
    <div class="card-front page-break">
        ...
        <div class="card-content">
            ...
            {{-- Nama Lengkap --}}
            <div class="full-name">{{ $p->nama_lengkap }}</div>

            {{-- Unit Kerja --}}
            <div class="unit-text">{{ $p->unit_kerja ?? 'Peserta Baitul Arqam' }}</div>

            {{-- QR Code --}}
            <div class="qr-area">
                <div class="qr-box">
                    <img src="data:image/svg+xml;base64,{{ base64_encode($qrc) }}" alt="QR Code">
                </div>
            </div>
        </div>
    </div>
