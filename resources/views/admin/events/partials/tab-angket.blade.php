...
<div x-data="angketManager()" x-init="init()">
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-xs text-gray-500 mt-0.5">8 kategori pertanyaan</p>
        </div>
        <button @click="showForm = true; editingItem = null; form = {kategori:'A', teks_item:''}"
    ...
    {{-- Category Groups --}}
    @foreach(['A','B','C','D','E','F','G','H'] as $kat)
            ...
            <div class="space-y-2">
                <template x-for="(item, idx) in itemsByKategori('{{ $kat }}')" :key="item.id">
                    <div class="flex items-center gap-3 bg-white rounded-lg border border-gray-100 px-4 py-3 group hover:shadow-sm transition-all">
                        ...
                        <p class="flex-1 text-sm text-gray-700" x-text="item.teks_item"></p>
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <select x-model="form.kategori" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none">
                        @foreach(['A','B','C','D','E','F','G','H'] as $k) <option value="{{ $k }}">Kategori {{ $k }}</option> @endforeach
                    </select>
                </div>
                <div>
...
function angketManager() {
    return {
        ...
        form: { kategori: 'A', teks_item: '' },

        init() {},
        itemsByKategori(k) { return this.allItems.filter(i => i.kategori === k); },

        editItem(item) { this.editingItem = item; this.form = { kategori: item.kategori, teks_item: item.teks_item }; this.showForm = true; },
