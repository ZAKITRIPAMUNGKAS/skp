...
<div x-data="ahpSawManager()" x-init="init()">
    ...
    <div x-show="subTab==='saw'">
        ...
        <template x-if="sawRows.length === 0 && !isCalculating">
            <div class="py-8"><x-empty-state title="Belum ada ranking" description="Klik 'Hitung Ranking' untuk menghitung ranking peserta." icon="chart" /></div>
        </template>
    </div>
</div>
...
function ahpSawManager() {
    return {
        ...
        async recalculate() {
            const res = await fetch('{{ route("admin.ahp.calculate", $event) }}', {
                method: 'POST',
        ...
        async saveBobot() {
            this.isSaving = true;
            await fetch('{{ route("admin.ahp.save", $event) }}', {
                method: 'POST',
                headers: {'Content-Type':'application/json','X-CSRF-TOKEN':'{{ csrf_token() }}'},
                body: JSON.stringify({ matrix: this.matrix }),
            });
        ...
        async loadExisting() {
            const res = await fetch('{{ route("admin.ahp.get", $event) }}');
        ...
        async calculateSaw(force = false) {
            ...
            const res = await fetch('{{ route("admin.saw.calculate", $event) }}', {
                method: 'POST',
                headers: {'Content-Type':'application/json','X-CSRF-TOKEN':'{{ csrf_token() }}'},
