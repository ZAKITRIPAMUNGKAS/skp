    ...
    return {
        ...
        async saveSoal(closeAfter) {
            const url = this.editingSoal
                ? '{{ route("admin.soal.update", [$event, "__SOAL__"]) }}'.replace('__SOAL__', this.editingSoal.id)
                : '{{ route("admin.soal.store", $event) }}';
        ...
        async deleteSoal(soal) {
            ...
            const url = '{{ route("admin.soal.destroy", [$event, "__SOAL__"]) }}'.replace('__SOAL__', soal.id);
        ...
        async handleDrop(event, targetSoal) {
            ...
            await fetch('{{ route("admin.soal.reorder", $event) }}', {
                method: 'POST',
