...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    @else
        {{-- Google Forms Style Layout Form --}}
        <div class="space-y-4" x-data="rtlFormManager()">
            {{-- Form Element --}}
            <form action="{{ route('peserta.rtl.submit', $event) }}" method="POST" id="rtlForm" enctype="multipart/form-data" class="space-y-4">
                @csrf
                
                {{-- Banner Top Card --}}
                <div class="bg-white rounded-3xl border-t-[10px] border-primary border-x border-b border-gray-150 p-6 sm:p-8 shadow-sm space-y-3">
                    <h2 class="text-2xl font-bold font-heading text-gray-800">Rencana Tindak Lanjut (RTL)</h2>
                    <p class="text-xs text-gray-500">Susun rencana aksi nyata Anda pasca pelatihan Baitul Arqam sebagai syarat penerbitan sertifikat resmi.</p>
                    <div class="text-[11px] text-red-500 font-medium">* Menunjukkan pertanyaan yang wajib diisi</div>
                </div>

                {{-- Card 2: Dynamic Questions --}}
                @foreach($rtlSoal as $soal)
                    @if($soal->urutan == 1)
                        <div class="bg-white rounded-3xl border border-gray-150 p-6 sm:p-8 shadow-sm">
                            <label class="block text-sm font-bold text-gray-800 mb-1">{{ $soal->pertanyaan }} <span class="text-red-500">*</span></label>
                            <p class="text-[11px] text-gray-400 mb-3">Maksimal ukuran file gambar adalah 5MB (Format: JPG, JPEG, PNG)</p>
                            <input type="file" name="answers[{{ $soal->id }}]" required accept="image/*"
                                   class="w-full text-sm text-gray-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20 file:cursor-pointer cursor-pointer border border-gray-200 rounded-xl p-2.5 bg-gray-50/50">
                        </div>
                    @else
                        @php
                            $displayTipe = $soal->tipe;
                        @endphp
                        <div class="bg-white rounded-3xl border border-gray-150 p-6 sm:p-8 shadow-sm">
                            @if($displayTipe === 'deskripsi')
                                <div class="bg-blue-50/50 border border-blue-100 p-5 rounded-2xl text-xs text-blue-700 leading-relaxed">
                                    <div class="flex items-start gap-2.5">
                                        <div class="p-1.5 bg-blue-100 rounded-lg text-blue-700 mt-0.5">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                        </div>
                                        <div>
                                            <p class="font-bold text-blue-900 mb-1">Informasi Pengisian:</p>
                                            {!! nl2br(e($soal->pertanyaan)) !!}
                                        </div>
                                    </div>
                                </div>
                            @elseif($displayTipe === 'essay')
                                <label class="block text-sm font-bold text-gray-800 mb-2">{{ $soal->pertanyaan }} <span class="text-red-500">*</span></label>
                                <textarea name="answers[{{ $soal->id }}]" required rows="4" placeholder="Tuliskan jawaban Anda secara detail..."
                ...
                <div class="flex justify-end items-center pt-2">
                    <button type="submit"
                            class="px-8 py-3.5 bg-gradient-to-r from-primary to-primary-700 hover:from-primary-600 hover:to-primary-800 text-white rounded-2xl text-sm font-bold shadow-lg shadow-primary/20 transition-all active:scale-95 flex items-center gap-2">
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
